...
## Server Function Validation (MEDIUM)

```typescript
// TanStack Start
export const createPostFn = createServerFn({ method: 'POST' })
  .inputValidator(createPostSchema)  // ✅ Always use Zod, never identity function
  .handler(async ({ data }) => {
...
- Always use `.inputValidator(schema)` in server functions — never identity functions
